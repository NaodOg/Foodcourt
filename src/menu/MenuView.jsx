  ...
  return (
    <header className="flex justify-between items-center w-full px-6 py-4 sticky top-0 z-50 bg-creamy dark:bg-zinc-950 border-b-2 border-surface-variant dark:border-zinc-800">
      <div className="flex items-center gap-3">
        ...
        <span className="font-['Plus_Jakarta_Sans'] font-bold uppercase tracking-tighter text-xl font-black text-brand-red dark:text-red-600 italic">
          {house?.name || "Menu"}
        </span>
      </div>
      <div
...
function CategoryNav({ tabs, activeTab, setActiveTab, onTabChange }) {
  ...
  return (
    <nav className="sticky top-[66px] z-40 bg-creamy/95 dark:bg-zinc-950/95 backdrop-blur-md py-4 -mx-6 px-6 mb-8 flex justify-center gap-3 border-b border-surface-variant dark:border-zinc-800 shadow-sm transition-colors duration-300 overflow-x-auto">
      {allTabs.map((tab) => (
        <button
          key={tab}
          onClick={() => {
          ...
          className={`px-6 py-2 rounded-full font-label-bold text-label-bold whitespace-nowrap transition-all border-2 border-brand-red active:scale-95 shrink-0 ${
            activeTab === tab
...
function SubCategoryCard({ subCategory, onClick }) {
  const itemCount = subCategory.itemCount ?? 0;
  return (
    <div
      onClick={onClick}
...
function CategoryItemsList({ houseId, subCategory, onBack, onAddToCart }) {
  const items = useQuery(api.menu.getItemsForCategory, { houseId, categoryId: subCategory.id });
...
export default function MenuView() {
  ...
  // Reset tab if current tab no longer exists
  const resolvedTab = (activeTab !== 'All' && settings && !tabs.includes(activeTab)) ? 'All' : activeTab;
  ...
  const currentCategories = useQuery(
    ...
    houseId ? (activeTab === 'All' ? { houseId } : { houseId, tab: activeTab }) : "skip"
  );
  ...
  if (house === undefined) {
    return (
      <div className="min-h-[100dvh] bg-creamy dark:bg-zinc-950 flex items-center justify-center">
  ...
  if (house === null) {
    return (
      <div className="min-h-[100dvh] bg-creamy dark:bg-zinc-950 flex flex-col items-center justify-center gap-4">
        <p className="text-brand-red font-label-bold text-lg">House not found</p>
        <button
          onClick={() => navigate('/')}
          className="bg-brand-red text-white font-label-bold px-6 py-3 rounded-xl hover:bg-brand-red/90 transition-colors"
        >
          Back to Flint Food Court
        </button>
      </div>
    );
  }
  ...
  return (
    <div className={`bg-creamy dark:bg-zinc-950 text-on-background dark:text-zinc-100 font-body-md min-h-[100dvh] transition-colors duration-300 ${totalItems > 0 ? 'pb-28' : 'pb-8'}`}>
      ...
      <main className="max-w-4xl mx-auto px-margin relative">
        ...
        {activeSubCategory ? (
          <CategoryItemsList houseId={houseId} subCategory={activeSubCategory} onBack={handleBack} onAddToCart={addToCart} />
      ...
      {totalItems > 0 && !isCartOpen && (
        <button
          ...
          className="fixed bottom-6 right-6 md:bottom-10 md:right-10 bg-brand-red text-white p-4 rounded-full shadow-xl hover:bg-brand-red/90 active:scale-95 transition-transform flex items-center justify-center z-40 group"
        >
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
              {isOrderPlaced ? (
                <div className="flex flex-col items-center animate-in fade-in duration-500">
                  ...
                  <p className="text-brand-red text-center mb-2 font-label-bold px-4 text-sm uppercase">
