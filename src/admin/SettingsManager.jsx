  ...
  return (
    <div>
      ...
      <p className="text-xs text-secondary mb-3">
        Each tab appears as a filter button on the public menu. "All" is always included by default.
      </p>
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <label className="font-label-bold text-xs text-on-background">Brand text</label>
            <input
              type="text"
              value={theme.brandRed}
              onChange={(e) => update("brandRed", e.target.value)}
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <label className="font-label-bold text-xs text-on-background">Background color (light mode)</label>
            <input
              type="text"
              value={theme.creamy}
              onChange={(e) => update("creamy", e.target.value)}
...
export default function SettingsManager() {
  ...
  const settings = useQuery(api.settings.getSettings, houseId ? { houseId } : "skip");
  const updateSettings = useMutation(api.settings.updateSettings);

  const [tabs, setTabs] = useState([]);
  const [taxRate, setTaxRate] = useState(15);
  const [theme, setTheme] = useState({ brandRed: "#E60000", creamy: "#F5EBE0" });
  ...
  const handleSave = async () => {
    setError("");
    if (!houseId) { setError("House not loaded"); return; }
    if (tabs.length === 0) { setError("At least one tab is required"); return; }
    if (taxRate < 0 || taxRate > 100) { setError("Tax rate must be between 0 and 100"); return; }

    setSaving(true);
    try {
      await updateSettings({ houseId, tabs, taxRate, theme });
      setSaved(true);
    } catch (err) {
  ...
  const isLoading = settings === undefined || house === undefined;
  ...
  return (
    <div className="max-w-2xl space-y-8">
      <div>
        <h2 className="font-headline-md text-xl text-on-background mb-1">Settings</h2>
        <p className="text-sm text-secondary">Control how the menu looks and behaves for {house?.name}.</p>
      </div>

      <section className="bg-white rounded-2xl border border-surface-variant p-6 space-y-6">
        <h3 className="font-label-bold text-sm uppercase tracking-wide text-secondary">Menu Tabs</h3>
        <TabEditor tabs={tabs} onChange={setTabs} />
      </section>

      <section className="bg-white rounded-2xl border border-surface-variant p-6 space-y-4">
        <h3 className="font-label-bold text-sm uppercase tracking-wide text-secondary">Tax</h3>
        <div>
          <label className="block font-label-bold text-sm mb-1.5 text-on-background">Tax Rate (%)</label>
          ...
          <p className="text-xs text-secondary mt-1.5">Applied to the subtotal in the customer's cart.</p>
        </div>
      </section>

      <section className="bg-white rounded-2xl border border-surface-variant p-6 space-y-4">
        <h3 className="font-label-bold text-sm uppercase tracking-wide text-secondary">Theme</h3>
