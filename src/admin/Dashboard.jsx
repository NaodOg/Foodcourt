...
export default function Dashboard() {
  ...
  const categories = useQuery(api.menu.getAllCategories, houseId ? { houseId } : "skip");
  const allItems = useQuery(api.menu.getAllItems, houseId ? { houseId } : "skip");

  const isLoading = categories === undefined || allItems === undefined || house === undefined;
  ...
  if (!house) {
    return (
      <div className="text-center py-20 text-secondary font-label-bold">
        House not found.
      </div>
    );
  }
  ...
  const stats = [
    {
      ...
      action: () => navigate(`/admin/${houseSlug}/categories`),
    },
    {
      label: 'Total Dishes',
      value: itemCount,
      ...
      action: () => navigate(`/admin/${houseSlug}/dishes`),
    },
    {
  ...
  return (
    <div className="space-y-8 max-w-5xl">
      {/* House header */}
      <div className="flex items-center gap-4 bg-white rounded-2xl border border-surface-variant p-5">
        <img src={house.logo} alt={house.name} className="w-14 h-14 rounded-xl object-cover shadow-sm" />
        <div>
          <h2 className="font-headline-md text-xl text-on-background">{house.name}</h2>
          <p className="text-sm text-secondary">{house.description}</p>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-3">
          <button
            onClick={() => navigate(`/admin/${houseSlug}/dishes`)}
          ...
          <button
            onClick={() => navigate(`/admin/${houseSlug}/categories`)}
